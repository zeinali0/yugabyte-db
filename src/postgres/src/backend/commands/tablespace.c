...
void validatePlacement(char *placement) {
	...
	if (rf_str == NULL) {
		ereport(ERROR,
			(errcode(ERRCODE_INVALID_PARAMETER_VALUE),
			errmsg("invalid value for \"placement\" option"),
			errdetail("%s where each block should have valid"
			" min_replicas_for_block ", err_detail)));
		return;
	}
...
DropTableSpace(DropTableSpaceStmt *stmt)
{
	...
	// Instead we have to go through the sys catalog tables to find whether
	// there are tables still associated with this tablespace. Alternatively
	// maybe explore using pg_depends while creating tables/indices?
#ifdef HAVE_SYMLINK
